...
export async function buildLocalBusinessGenome(location,{caches:providedCaches,config:providedConfig,presentationDemo=false}={}){
  ...
  const density=densityValue===null?unavailable('msme_density',{name:'MSME density derived from UDYAM and district Census data',publisher:'VyaparGuide calculation'},'DISTRICT',districtName,'Requires verified district-level UDYAM total and district-level Census population; no geographic levels are mixed.'):evidence(densityValue,{dataset:'msme_density',config:{name:'MSMEs per 1,000 district population',publisher:'VyaparGuide calculation'},cache:udyamTotalCache,geographyLevel:'DISTRICT',geographyName:districtName,status:'DERIVED',detail:'Formula: (district UDYAM registered enterprises / district Census population) × 1,000.'});
