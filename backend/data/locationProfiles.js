...
export function locationProfile(district,village,coords=[],subdistrict=null,villageCode=null){
  ...
  const v=matches[0];
  return{state:v.state,stateCode:v.stateCode,district:v.district,districtCode:v.districtCode,subdistrict:v.subdistrict,subdistrictCode:v.subdistrictCode,village:v.village,villageCode:v.villageCode,pincode:v.pincode,...point,...emptyGenomeFields(),dataType:'VERIFIED_LOCATION',dataSource:'Local Government Directory (LGD)',sourcePublisher:'Government of India',sourceDataset:'Villages with PIN Codes',sourceStatus:'Village identity and administrative information verified against LGD.'};
}
